...
***********************************************************************/
NTSTATUS
MD5File(IN PUNICODE_STRING pStrFileName, OUT PUNICODE_STRING pStrFileMD5, PFLT_FILTER FileObject, PFLT_INSTANCE Instance)
{
    ...
    // PDEVICE_OBJECT pDevice = IoGetRelatedDeviceObject(FileObject);

    status = FltCreateFile(FileObject, Instance, &hFile, GENERIC_READ, &objAttributes,
        &ioStatus, NULL, FILE_ATTRIBUTE_NORMAL,
        FILE_SHARE_READ,
        FILE_OPEN,
        FILE_SYNCHRONOUS_IO_NONALERT, NULL,
        0, 0);
